...
export function createTask(input) {
	...
	return API_SERVICE.MYTASK_API.mutate({
		...
		.then(result => {
			return store.dispatch({ type: TASK.CREATE_SUCCESS, payload: result.data.createTask });
		})
		.catch(err => {
			store.dispatch({ type: TASK.IMPORT_ALL_FAILED, payload: err });
		});
}
...
export function deleteTask(input) {
	...
	return API_SERVICE.MYTASK_API.mutate({
		...
		.then(result => {
			return store.dispatch({ type: TASK.DELETE_SUCCESS, payload: result.data.deleteTask });
		})
		.catch(err => {
			console.log('err', err);
